...
void stateAdjust(int inputValue, struct gameState *state) {
	state->supplyCount[estate] = inputValue;
	state->supplyCount[duchy] = inputValue;
	state->supplyCount[province] = inputValue;
}
...
{
	int card, coin_bonus = 0; 		//tracks coins gain from actions
...
int getWinners(int players[MAX_PLAYERS], struct gameState *state) {
	int i, j, highScore, currentPlayer;
...
int drawCard(int player, struct gameState *state)
{
	...
	if (state->deckCount[player] <= 0) {//Deck is empty
	  //Step 1 Shuffle the discard pile back into a deck
		...
		if (deckCounter == 0)
			return -1;
		adjustHand(player, state, deckCounter, count);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case mine:
		output = callMine(choice1, currentPlayer, choice2, state, handPos);
		return output;

	case minion:
		output = callMinion(currentPlayer, choice1, choice2, state, handPos);
		return output;

	case tribute:
		output = callTribute(currentPlayer, nextPlayer, state);
		return output;

	case ambassador:
		output = callAmbassador(currentPlayer, choice1, choice2, state, handPos);
		return output;

	case baron:
		output = callBaron(choice1, currentPlayer, state);
		return output;
...
int callMinion(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos) {
	int i = 0;
	int j = 0;
	state->numActions++;
	//discard card from hand
...
int callAmbassador(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos) {
...
int callTribute(int currentPlayer, int nextPlayer, struct gameState *state) {
	int i = 0;
	int tributeRevealedCards[2] = { -1, -1 };
...
int callMine(int choice1, int currentPlayer, int choice2, struct gameState *state, int handPos) {
	int i = 0;
	int j = 0;
