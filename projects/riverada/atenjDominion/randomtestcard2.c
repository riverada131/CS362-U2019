...
int main() {
	...
	for (i = 0; i < MAX_NUM_TEST; i++) {
		...
		cardEffect(minion, choice1, choice2, choice3, &test_state, handPos, &bonus);
		...
		if (choice1 == 1 && choice2 == 0) {
			extra_coins = 4; // add 4 since we are adding 2 to previous amount which had 2 added
		}
		else if (choice1 == 1 && choice2 == 1) {
			extra_coins = 4; // add 4 since we are adding 2 to previous amount which had 2 added
		}
		else if (choice1 == 0 && choice2 == 1) {
			extra_coins = 2; // same value as before. coin count is unchanged
			upper_limit = 4;
		}
